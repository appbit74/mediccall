$(document).ready(function() {
    ...
    function updateView(role, data) {
        switch (role) {
            case 'counter':
                ...
                break;
            case 'therapist':
                const allCurrentTherapistIds = new Set([...(data.new_patients || []).map(p => p.id), ...(data.waiting_therapy || []).map(p => p.id), ...(data.in_therapy || []).map(p => p.id)]);
                const newTherapistPatientIds = new Set([...allCurrentTherapistIds].filter(id => !previousState.therapistAllIds.has(id)));
                if (newTherapistPatientIds.size > 0) { playSound('notification-sound'); }
                previousState.therapistAllIds = allCurrentTherapistIds;
                populateColumn('#new-patients-col', data.new_patients, (p) => createTherapistWaitingCard(p, false), 'ไม่มีคนไข้ใหม่');
                populateColumn('#waiting-therapy-col', data.waiting_therapy, (p) => createTherapistWaitingCard(p, newTherapistPatientIds.has(p.id)), 'ไม่มีคนไข้รอทำกายภาพ');
                populateColumn('#in-therapy-col', data.in_therapy, (p) => createTherapistInTherapyCard(p, newTherapistPatientIds.has(p.id)), 'ไม่มีคนไข้กำลังทำกายภาพ');
                break;
            case 'doctor':
                const currentDoctorWaitingIds = new Set((data.my_patients || []).filter(p => p.status === 'waiting_doctor').map(p => p.id));
                const newDoctorPatientIds = new Set([...currentDoctorWaitingIds].filter(id => !previousState.doctorWaitingIds.has(id)));
                ...
                if (!data.my_patients || data.my_patients.length === 0) { list.html('<div class="list-group-item">ไม่มีคนไข้ในคิวของคุณ</div>'); return; }
                $.each(data.my_patients, (i, p) => { const isNew = newDoctorPatientIds.has(p.id); list.append(createDoctorCard(p, isNew)); });
                break;
        }
    }
    ...
    function createDoctorCard(p, isNew = false) {
        let statusText = '';
        switch(p.status) { case 'waiting_therapy': statusText = 'รอทำกายภาพ'; break; case 'in_therapy': statusText = 'กำลังทำกายภาพ'; break; case 'waiting_doctor': statusText = 'รอตรวจ'; break; default: statusText = p.status; }
    ...
    $('#manual-sync-btn').on('click', function() { const btn = $(this); btn.prop('disabled', true).html('<span class="spinner-border spinner-border-sm"></span> Syncing...'); $.getJSON('api/data_handler.php?action=manual_sync').always(() => btn.prop('disabled', false).html('<i class="bi bi-arrow-clockwise"></i> Sync JERA')); });
    ...
    $(document).on('click', '.btn-complete-payment', function() { handleAction('complete_payment', { patient_id: $(this).data('id') }); });
    $(document).on('click', '.btn-send-back-to-therapy', function() { handleAction('send_back_to_therapy', { patient_id: $(this).data('id') }); });
    $(document).on('click', '.btn-therapist-finish-work', function() { if (confirm('ยืนยันการจบงาน และส่งต่อแผนกการเงิน?')) { handleAction('therapist_finish_work', { patient_id: $(this).data('id') }); } });
    $(document).on('click', '.btn-assign-doctor', function() { $('#doctor-modal-patient-id').val($(this).data('id')); $('#doctor-modal-patient-name').text($(this).data('name')); $.getJSON('api/data_handler.php?get=doctors', (doctors) => { const sel = $('#doctor-select').empty(); $.each(doctors, (i, d) => sel.append(`<option value="${d.id}">${d.name}</option>`)); }); new bootstrap.Modal('#assignDoctorModal').show(); });
    $('#confirm-assign-doctor').on('click', function() { handleAction('assign_doctor', { patient_id: $('#doctor-modal-patient-id').val(), doctor_id: $('#doctor-select').val(), doctor_name: $('#doctor-select option:selected').text() }, bootstrap.Modal.getInstance($('#assignDoctorModal'))); });
